...
/**
 * Normaliza un nodo que puede venir como objeto único o arreglo.
 * @param {*} node - Nodo del árbol XML.
 * @returns {Array<object>} Arreglo (posiblemente vacío).
 */
...
/**
 * Lee un atributo de un nodo del árbol de fast-xml-parser.
 * @param {object} node - Nodo XML.
 * @param {string} name - Nombre del atributo (sin prefijo).
 * @returns {string|undefined} Valor del atributo o undefined.
 */
...
/**
 * Convierte un atributo numérico a Number; undefined si no existe.
 * @param {object} node - Nodo XML.
...
/**
 * Busca un hijo por nombre local, tolerando prefijos de namespace distintos
 * (cfdi:, tfd:, pago20:, nomina12:, o sin prefijo).
 * @param {object} node - Nodo padre.
 * @param {string} localName - Nombre local del elemento (p. ej. "Emisor").
 * @returns {*} El hijo encontrado o undefined.
 */
...
/**
 * Igual que child() pero siempre devuelve arreglo.
 * @param {object} node - Nodo padre.
 * @param {string} localName - Nombre local del elemento.
...
/**
 * Núcleo del parseo: opera sobre el árbol ya construido, sin depender de quién
 * lo construyó.
 *
 * Existe para que el mismo código corra en Node (con fast-xml-parser) y en el
 * navegador (con DOMParser, ver cfdiBrowser.js) sin duplicar la lógica fiscal.
 * El contrato del árbol es el de fast-xml-parser: atributos con prefijo `@_`,
 * hijos por nombre de etiqueta, y arreglo cuando la etiqueta se repite.
 *
 * @param {object} tree - Árbol XML con el contrato descrito.
 * @returns {object} El mismo shape que parseCfdi().
 * @throws {CfdiParseError}
 */
export function parseCfdiDesdeArbol(tree) {
  // Raíz: cfdi:Comprobante (tolerante al prefijo).
  ...
  // ---- Impuestos globales (nodo cfdi:Impuestos hijo directo del Comprobante) ----
  ...
  // ---- CfdiRelacionados (puede haber varios nodos, uno por TipoRelacion) ----
  ...
  // ---- Complemento: TimbreFiscalDigital, Pagos 2.0, Nómina 1.2 ----

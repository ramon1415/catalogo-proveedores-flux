...
export function ProviderModal({
  ...
  // Precarga desde la CSF: parseo local del PDF (RFC, razón social/nombre,
  // tipo de persona por longitud del RFC). Solo rellena campos vacíos; si el
  // PDF es un escaneo sin texto, la captura queda manual sin ruido.
  async function onCsfFile(file: File | null) {
    ...
    if (!csf) { setCsfParseHint('No se pudo leer la CSF (¿escaneo?). Captura los datos manualmente.'); return }
    setF((prev) => ({
      ...prev,
      rfc: prev.rfc || (csf.rfc ?? ''),
      nombre_completo: prev.nombre_completo || (csf.nombre ?? ''),
      persona_tipo: prev.persona_tipo || (csf.personaTipo ?? ''),
      notas: prev.notas || [csf.regimen && `Régimen: ${csf.regimen}`, csf.codigoPostal && `CP: ${csf.codigoPostal}`, csf.idCif && `idCIF: ${csf.idCif}`].filter(Boolean).join(' · '),
    }))
          ...
          <div className={s.formGrid}>
            ...
            <label className={s.fullRow}>Constancia de Situación Fiscal (CSF)
              <input type="file" accept="application/pdf,image/*" disabled={dis || !canUploadCsf}
                onChange={(e) => onCsfFile(e.target.files?.[0] ?? null)} />

...
export async function loadActiveProfiles(): Promise<Profile[]> {
  const { data, error } = await supabase
    .from('profiles')
    .select('id,full_name,email,active')
    .order('full_name', { ascending: true })
  if (error) return []
  return ((data ?? []) as Array<Profile & { active: boolean | null }>)
...
export async function loadEmployeeBankAccount(profileId: string): Promise<EmployeeBankAccount | null> {
  const { data, error } = await supabase
    .from('employee_bank_accounts')
    .select('profile_id,banco,clabe,cuenta,beneficiary_name,updated_at')
...
export async function upsertEmployeeBankAccount(account: EmployeeBankAccount): Promise<void> {
  const { error } = await supabase
    .from('employee_bank_accounts')
    .upsert({ ...account, updated_at: new Date().toISOString() }, { onConflict: 'profile_id' })
  if (error) throw error
}
...
export async function loadReimbursementItems(requestId: string): Promise<ReimbursementItem[]> {
  const { data, error } = await supabase
    ...
    .select('id,payment_request_id,budget_category_id,descripcion,amount,subtotal_amount,tax_amount,deducible,invoice_uuid,storage_path,created_at')

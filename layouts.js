...
const BBVA_INTERBANK_REFERENCE_FIELD_LENGTH = 7
const BBVA_INTERBANK_REFERENCE_INPUT_RULE = "1 a 5 digitos; el TXT completa la referencia a 7 digitos"
const BBVA_INTERBANK_BANK_FIELD_PREFIX = "40"
const BBVA_INTERBANK_BANK_CODE_LENGTH = 3
const BBVA_INTERBANK_BANK_FIELD_LENGTH = 5
const BBVA_INTERBANK_CONCEPT_LENGTH = 30
const BBVA_INTERBANK_INDICATOR = "H"
const BBVA_INTERBANK_LINE_LENGTH = CXC_ACCOUNT_LENGTH * 2 + CXC_CURRENCY_LENGTH + CXC_AMOUNT_LENGTH + BBVA_INTERBANK_BENEFICIARY_LENGTH + BBVA_INTERBANK_BANK_FIELD_LENGTH + BBVA_INTERBANK_CONCEPT_LENGTH + BBVA_INTERBANK_REFERENCE_FIELD_LENGTH + 1
const BBVA_INTERBANK_LINE_PATTERN = /^\d{18}\d{18}MXP\d{13}\.\d{2}[A-Z0-9 .,&\/-]{30}\d{5}[A-Z0-9 .,&\/-]{30}\d{7}H$/
const BBVA_CIE_CONCEPT_LENGTH = 30
const BBVA_CIE_CONVENIO_LENGTH = 7
const BBVA_CIE_REFERENCE_LENGTH = 20
const BBVA_CFE_CONVENIO = "0578869"
const BBVA_CIE_LINE_LENGTH = BBVA_CIE_CONCEPT_LENGTH + BBVA_CIE_CONVENIO_LENGTH + CXC_ACCOUNT_LENGTH + CXC_AMOUNT_LENGTH + BBVA_CIE_CONCEPT_LENGTH + BBVA_CIE_REFERENCE_LENGTH
const BBVA_CIE_LINE_PATTERN = /^[\x20-\x7e]{30}\d{7}\d{18}\d{13}\.\d{2}[\x20-\x7e]{30}[\x20-\x7e]{20}$/
const BBVA_CIE_TRASH_CHARACTERS = "áéíóúÁÉÍÓÚ.ñÑ!#$%&/()='?¿¡"
const BBVA_CIE_REPLACEMENT_CHARACTERS = "aeiouAEIOU nN             "
...
let activeCreatedLayout = null
let layoutCreationSubmitting = false
let layoutCreationRequestSequence = 0
...
function cacheDom() {
  ...
  dom.layoutCreatedResult = document.getElementById("layoutCreatedResult")
...
function bindEvents() {
  ...
  dom.layoutCreatedResult?.addEventListener("click", handleCreatedLayoutAction)
  ...
  dom.newLayoutDialog?.addEventListener("cancel", (event) => {
    if (layoutCreationSubmitting) {
...
function renderFormatDownloadActions(layout, summary) {
  ...
  if (cie.count > 0) {
    if (cie.validationIssues > 0) {
      actions.push(`<button class="small-btn warning" type="button" onclick="openLayoutLines('${layout.id}')" style="white-space:nowrap">Revisar CIE (${cie.validationIssues})</button>`)
    } else {
      actions.push(`<button class="small-btn" type="button" onclick="downloadLayoutBbvaFormat('${layout.id}','${BBVA_FORMAT_CIE}')" style="white-space:nowrap">▾ Descargar CIE</button>`)
    }
  }
...
function closeNewLayoutModal() {
  ...
  layoutCreationRequestSequence += 1
...
function resetNewLayoutForm() {
  ...
  activeCreatedLayout = null
  layoutCreationSubmitting = false
  layoutCreationRequestSequence += 1
  dom.layoutEligibilityPreview.classList.add("hidden")
  dom.layoutEligibilityPreview.innerHTML = ""
  dom.layoutCreatedResult?.classList.add("hidden")
  if (dom.layoutCreatedResult) dom.layoutCreatedResult.innerHTML = ""
  setNewLayoutFormLocked(false)
...
async function reviewLayoutEligibility() {
  if (!ensureActorProfile()) return
  if (activeCreatedLayout || layoutCreationSubmitting) return
  ...
  try {
    ...
    if (providerIds.length) {
      const { data: providers, error: providerError } = await supabaseClient.from('proveedores').select('id,convenio_number').in('id', providerIds)
      if (providerError) throw providerError
      preview = withCiePreviewConvenios(preview, providers || [])
    }
    if (requestId !== activeLayoutPreviewRequestId || paramsKey !== layoutPreviewParamsKey()) return
    layoutEligibilityPreview = applyCieReferencePreflight(preview)
...
function renderLayoutEligibilityPreview() {
  ...
  dom.submitNewLayoutBtn.textContent = ready.length
    ? `Crear y descargar layout con ${ready.length} ${ready.length === 1 ? "pago" : "pagos"}`
    : noReadyMessage
  dom.submitNewLayoutBtn.title = ready.length
    ? `Crear layout con ${ready.length} ${ready.length === 1 ? "pago" : "pagos"} y preparar su descarga`
    : noReadyMessage
  resetLayoutPreviewScrollPositions()
}
...
function renderPreviewRow(row, kind) {
  ...
  if (kind === "ready" && row.classification === "ready_extraordinary") {
    ...
    if (missing.some((field) => providerOwnedLayoutFields().includes(field)) && row.proveedor_id) {
...
function providerOwnedLayoutFields() {
  return [
    "proveedor_id",
    "proveedor_not_found",
    "proveedor_inactive",
...
function openLayoutCompletionDialog(requestId) {
  ...
  if (request.destination_type === "convenio") {
    dom.layoutCompletionReference.removeAttribute("maxlength")
    dom.layoutCompletionReference.placeholder = "Copia la referencia completa del recibo"
    dom.layoutCompletionReferenceHint.textContent = isCfeCieConvenio(previewCieConvenio(request))
      ? "CFE: línea de captura de 20 caracteres exactos del recibo, sin espacios. No uses una fecha ni el número de servicio."
      : "Copia la referencia del recibo, hasta 20 caracteres. No se recorta ni se completa con ceros."
  }
...
async function submitLayoutCompletion(event) {
  ...
  if (activeLayoutCompletionRequest.destination_type === "convenio") {
    const cieError = cieReferenceError(reference, previewCieConvenio(activeLayoutCompletionRequest))
    if (cieError) { showToast("Revisa la referencia CIE", cieError, "warning"); dom.layoutCompletionReference.focus(); return }
  }
...
async function submitNewLayout(event) {
  event.preventDefault()
  if (!ensureActorProfile()) return
  if (activeCreatedLayout) {
    renderCreatedLayoutResult()
    scrollLayoutModalToSection(dom.layoutCreatedResult)
    return
  }
  if (layoutCreationSubmitting) return
  ...
  const requestId = ++layoutCreationRequestSequence
  layoutCreationSubmitting = true
  setNewLayoutCreationBusy(true)
  ...
  try {
    ...
    const layoutId = cleanText(data?.layout_id)
    if (!layoutId) throw new Error("layout_created_without_id")

    activeCreatedLayout = {
      id: layoutId,
      number: data?.layout_number || "Layout",
      paymentCount: numberValue(data?.payment_count),
      layout: null,
      lines: [],
    ...
    layoutEligibilityPreview = null
    layoutEligibilityPreviewParamsKey = null
    setNewLayoutFormLocked(true)

    await loadLayouts()
    const layout = layouts.find((item) => item.id === layoutId)
    if (!layout) throw new Error("layout_created_not_readable")

    const { data: lines, error: linesError } = await fetchLayoutLines(layoutId)
    if (linesError) throw linesError
    if (requestId !== layoutCreationRequestSequence || !dom.newLayoutDialog?.open) return

    activeCreatedLayout.number = data?.layout_number || layout.layout_number || "Layout"
    activeCreatedLayout.layout = layout
    activeCreatedLayout.lines = lines || []
    ...
    renderLayoutNotice(`Layout ${activeCreatedLayout.number} con ${activeCreatedLayout.paymentCount} pagos. ${summary}`, data?.invalid_requests || [])
    renderCreatedLayoutResult()
    showToast(
      "Layout creado",
      summary,
    ...
    const autoFormat = singleReadyCreatedLayoutFormat(activeCreatedLayout.lines)
    if (autoFormat) {
      activeCreatedLayout.autoDownloaded = true
      renderCreatedLayoutResult()
      await downloadLayoutBbvaFormat(layoutId, autoFormat)
    }
  } catch (error) {
    if (requestId !== layoutCreationRequestSequence) return
    if (activeCreatedLayout) {
      renderCreatedLayoutResult()
      renderLayoutNotice(`El Layout ${activeCreatedLayout.number} ya fue creado. No se volverá a crear; reintenta la descarga desde este mismo resultado.`)
      showToast("Layout creado", "No se pudo preparar la descarga automática. El Layout no se duplicará.", "warning")
    } else {
      renderLayoutNotice(friendlyRpcError(error))
      showToast("No se pudo crear layout", friendlyRpcError(error), "danger")
    }
  } finally {
    if (requestId === layoutCreationRequestSequence) {
      layoutCreationSubmitting = false
      setNewLayoutCreationBusy(false)
    }
    setButtonLoading(dom.submitNewLayoutBtn, false, "Crear layout")
    if (activeCreatedLayout) {
      dom.submitNewLayoutBtn.disabled = true
      dom.submitNewLayoutBtn.textContent = `${activeCreatedLayout.number} creado`
    } else if (!layoutEligibilityPreview) {
      dom.submitNewLayoutBtn.disabled = true
      dom.submitNewLayoutBtn.textContent = "Revisa solicitudes primero"
    }
  }
...
function setNewLayoutCreationBusy(busy) {
  if (dom.closeNewLayoutModalBtn) dom.closeNewLayoutModalBtn.disabled = busy
  if (dom.cancelNewLayoutBtn) dom.cancelNewLayoutBtn.disabled = busy
  if (dom.reviewLayoutBtn) dom.reviewLayoutBtn.disabled = busy || Boolean(activeCreatedLayout)
}

function setNewLayoutFormLocked(locked) {
  ;[
    dom.layoutPeriodStart,
    dom.layoutPeriodEnd,
    dom.layoutName,
    dom.layoutCompanyId,
    dom.layoutBankAccountId,
  ].forEach((field) => {
    if (field) field.disabled = locked
  })
  if (dom.reviewLayoutBtn) dom.reviewLayoutBtn.disabled = locked
}
...
function createdLayoutDownloadFormats(lines) {
  ...
  const formats = [BBVA_FORMAT_SAME_BANK, BBVA_FORMAT_MIXED, BBVA_FORMAT_INTERBANK, BBVA_FORMAT_CIE].filter((format) => {
    ...
    return selected.length > 0 && validateLayoutLines(selected).length === 0
  })
  return formats
}
...
function singleReadyCreatedLayoutFormat(lines) {
  ...
  const formats = createdLayoutDownloadFormats(activeLines)
  return formats.length === 1 ? formats[0] : null
}

function renderCreatedLayoutResult() {
  if (!dom.layoutCreatedResult || !activeCreatedLayout) return
  const formats = createdLayoutDownloadFormats(activeCreatedLayout.lines)
  const actions = formats.map((format) => (
    `<button type="button" class="primary-btn" data-created-layout-action="download" data-format="${escapeHtml(format)}">Descargar ${escapeHtml(bbvaFormatLabel(format))}</button>`
  ))
  if (!actions.length) {
    actions.push(`<button type="button" class="secondary-btn" data-created-layout-action="details">Revisar líneas</button>`)
  }
  const downloadMessage = activeCreatedLayout.autoDownloaded
    ? "La descarga automática inició. Puedes descargar nuevamente sin crear otro Layout."
    : formats.length > 1
      ? "Selecciona el formato bancario que necesitas."
      : "El Layout quedó creado; usa la descarga disponible sin volver a crearlo."
  dom.layoutCreatedResult.innerHTML = `
    <h3>${escapeHtml(activeCreatedLayout.number)} creado con ${activeCreatedLayout.paymentCount} ${activeCreatedLayout.paymentCount === 1 ? "pago" : "pagos"}</h3>
    <p>${escapeHtml(downloadMessage)}</p>
    <div class="layout-created-actions">${actions.join("")}</div>`
  dom.layoutCreatedResult.classList.remove("hidden")
}

async function handleCreatedLayoutAction(event) {
  const button = event.target.closest("[data-created-layout-action]")
  if (!button || !activeCreatedLayout) return
  if (button.dataset.createdLayoutAction === "details") {
    await openLayoutLines(activeCreatedLayout.id)
    return
  }
  if (button.dataset.createdLayoutAction === "download") {
    await downloadLayoutBbvaFormat(activeCreatedLayout.id, button.dataset.format)
  }
}
...
function renderLinesTable(lines) {
  ...
  dom.linesTableBody.innerHTML = lines.map((line) => `
    <tr>
      <td>${escapeHtml(layoutSourceAccountDisplay(line))}</td>
      <td>${escapeHtml(line.company_name || "")}</td>
      <td>${escapeHtml(layoutDestinationDisplay(line))}</td>
...
function renderFormatSummaryRow(item, key) {
  ...
  if (key === BBVA_FORMAT_SAME_BANK) {
    ...
    if (item.validationIssues > 0) {
      status = `<span class="badge warning">${item.validationIssues} linea(s) CIE por corregir</span>`
      action = `<span style="color:var(--text-2);font-size:12px">Revisa convenio, referencia, cuenta, importe y concepto</span>`
    } else {
      action = `<button class="small-btn" type="button" onclick="downloadLayoutBbvaFormat('${activeLinesLayoutId}','${BBVA_FORMAT_CIE}')">▾ Descargar CIE</button>`
    }
  } else {
...
function renderLineActions(line) {
  ...
  const actions = []
  if (line.destination_type === "convenio") {
    actions.push(`<button class="small-btn warning" type="button" onclick="openCieReferenceModal('${line.id}')">Corregir referencia CIE</button>`)
  }
...
function renderLineReferenceCell(line) {
  ...
  if (line.status === "included" && line.destination_type === "convenio") {
    const issue = cieReferenceError(line.payment_reference, line.convenio_number)
    return `${value || "Sin referencia"}${issue ? `<small style="display:block;max-width:260px">${escapeHtml(issue)}</small>` : ""}`
  }
...
function openCieReferenceModal(lineId) {
  if (!ensureActorProfile()) return
  const line = activeLayoutLines.find(item => item.id === lineId)
  const layout = layouts.find(item => item.id === line?.layout_id)
  if (!line || line.status !== "included" || line.destination_type !== "convenio" || !layout || ["confirmed", "cancelled"].includes(layout.status)) return
  const dialog = document.createElement("dialog")
  dialog.className = "narrow"
  dialog.innerHTML = `<form class="modal-content"><div class="modal-header"><div><small data-company></small><h2>Corregir referencia CIE</h2><p data-request></p></div></div><div class="modal-scroll"><div class="form-grid"><label class="full-row">Línea de captura / referencia del recibo<input data-reference type="text" autocomplete="off" spellcheck="false" required><span class="field-hint" data-hint></span><span class="field-hint">Copia el dato del recibo. No uses una fecha ni el número de servicio; no agregues ceros.</span></label><label class="full-row" data-confirm-label><input data-confirm type="checkbox" style="width:auto"> Confirmo que el banco rechazó este pago y no lo ejecutó.</label><p class="full-row" role="alert" data-error></p></div></div><div class="modal-actions"><button type="button" class="secondary-btn" data-cancel>Cancelar</button><button type="submit" class="primary-btn" data-save>Guardar referencia</button></div></form>`
  dialog.querySelector("[data-company]").textContent = line.company_name || ""
  dialog.querySelector("[data-request]").textContent = `${line.request_number || ""} · Convenio ${line.convenio_number || ""}`
  const input = dialog.querySelector("[data-reference]")
  const confirmation = dialog.querySelector("[data-confirm]")
  const errorNode = dialog.querySelector("[data-error]")
  const save = dialog.querySelector("[data-save]")
  const cancel = dialog.querySelector("[data-cancel]")
  const uploaded = layout.status === "uploaded"
  dialog.querySelector("[data-confirm-label]").hidden = !uploaded
  input.value = line.payment_reference || ""
  let saving = false
  const validate = () => {
    const issue = cieReferenceError(input.value, line.convenio_number)
    errorNode.textContent = issue || ""
    dialog.querySelector("[data-hint]").textContent = `${input.value.trim().length} / 20 caracteres. ${isCfeCieConvenio(line.convenio_number) ? "CFE exige exactamente 20, sin espacios." : "Hasta 20, según el convenio."}`
    save.disabled = saving || Boolean(issue) || (uploaded && !confirmation.checked)
  }
  const close = () => { if (!saving) { dialog.close(); dialog.remove() } }
  cancel.onclick = close
  dialog.addEventListener("cancel", event => { event.preventDefault(); close() })
  input.oninput = validate
  confirmation.onchange = validate
  dialog.querySelector("form").onsubmit = async event => {
    event.preventDefault()
    if (saving || cieReferenceError(input.value, line.convenio_number) || (uploaded && !confirmation.checked)) return
    saving = true; cancel.disabled = true; input.disabled = true; confirmation.disabled = true; validate()
    try {
      const reference = input.value.trim()
      const { error } = await supabaseClient.rpc("update_payment_layout_line_cie_reference", { p_line_id: line.id, p_payment_reference: reference, p_expected_reference: line.payment_reference, p_bank_rejection_confirmed: confirmation.checked })
      if (error) throw error
      await refreshLayoutLines(line.layout_id)
      if (activeLayoutLines.find(item => item.id === line.id)?.payment_reference !== reference) throw new Error("La referencia no se confirmó al recargar. Revisa la línea antes de descargar.")
      await loadLayouts()
      showToast("Referencia CIE guardada", "Vuelve a descargar el archivo CIE de este layout.", "success")
      saving = false; close()
    } catch (error) {
      errorNode.textContent = cieReferenceSaveError(error)
      showToast("No se pudo guardar", cieReferenceSaveError(error), "danger")
    } finally { saving = false; cancel.disabled = false; input.disabled = false; confirmation.disabled = false; save.disabled = false }
  }
  document.body.appendChild(dialog)
  validate(); dialog.showModal(); input.focus()
}
...
window.openCieReferenceModal = openCieReferenceModal
...
function buildBbvaCieContent(lines) {
...
function serializeBbvaCieLine(line) {
  ...
  if (row.length !== BBVA_CIE_LINE_LENGTH) {
    throw new Error(`cie_line_length_invalid_${row.length}`)
  }
...
function isCfeCieConvenio(value) {
    const convenio = String(value ?? '').trim();
    return /^\d{6,7}$/.test(convenio) && convenio.padStart(7, '0') === BBVA_CFE_CONVENIO;
}
function cieReferenceError(value, convenio) {
    const reference = String(value ?? '').trim();
    if (!reference)
        return 'La referencia CIE es obligatoria. Copia la línea de captura del recibo.';
    if (reference.length > BBVA_CIE_REFERENCE_LENGTH)
        return 'La referencia CIE excede 20 caracteres. Revisa el recibo; no se recorta automáticamente.';
    if (!/^[\x20-\x7e]+$/.test(reference))
        return 'La referencia CIE debe usar caracteres ASCII portables, sin acentos ni saltos de línea.';
    if (reference.includes('|'))
        return 'La referencia CIE contiene el caracter | no permitido.';
...
function withCiePreviewConvenios(preview, providers) {
    const convenios = new Map(providers.map((provider) => [provider.id, provider.convenio_number]));
    return Object.fromEntries(Object.entries(preview).map(([key, rows]) => [key, Array.isArray(rows) ? rows.map((row) => normalizeDestinationType(row.destination_type) === 'convenio' && row.proveedor_id
            ? { ...row, convenio_number: convenios.get(row.proveedor_id) ?? null }
            : row) : rows]));
}
function applyCieReferencePreflight(preview) {
    const result = { ...preview, invalid_data: [...(preview.invalid_data || [])] };
    for (const key of ['ready_regular', 'ready_extraordinary', 'legacy_eligible']) {
        result[key] = (preview[key] || []).filter((row) => {
            if (normalizeDestinationType(row.destination_type) !== 'convenio')
                return true;
...
function formatBbvaCieConvenio(value) {
...
function formatBbvaCieSourceAccount(value) {
  const input = String(value ?? "").trim()
...
function formatBbvaCieAmount(value) {
...
function replaceBbvaCieTrash(value) {
...
function normalizeBbvaCieText(value) {
  return replaceBbvaCieTrash(value).toUpperCase()
}

function formatBbvaCieText(value, length, label) {
...
function validateBbvaCieContent(content) {
  return validateBbvaContent(content, {
    formatLabel: "CIE",
    lineLength: BBVA_CIE_LINE_LENGTH,
    linePattern: BBVA_CIE_LINE_PATTERN,
    validateLine: validateBbvaCieFields,
  })
}
...
function validateBbvaCieFields(line, lineNumber, errors) {
...
function parseBbvaCieLine(line) {
  return {
...
function maskSensitiveSuffix(value, visible = 4) {

...
async function openProviderFromQuery() {
  ...
  if (!providerId) {
    if (!canManageProviders()) {
      showToast("Sin permiso", "La administración de proveedores corresponde a un usuario interno autorizado.", "warning")
      return
    }
    openCreateModal()
  ...
  else if (intakeProposal) showIntakeProposal(true)
}
...
function applyIntakeProposal(requireConfirmation) {
  if (!intakeProposal) return
  if (requireConfirmation && !confirm("Aplicar los datos declarados como propuesta editable? Ningún cambio se guardará hasta que confirmes Guardar proveedor.")) return

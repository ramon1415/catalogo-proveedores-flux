...
function assertRequiredEnv() {
  ...
  if (!process.env.N8N_DEV_API_URL) {
    fail('N8N_DEV_API_URL is required.');
  }
  if (!process.env.N8N_DEV_API_KEY) {
    fail('N8N_DEV_API_KEY is required.');
  }
}
...
function getApiBase() {
  ...
  if (/(^|[._-])(prod|production|main)([._-]|$)/.test(normalizedSignals)) {
    fail('A PROD/main signal was detected in N8N_DEV_API_URL or environment. Aborting DEV import.');
  }
...
function resolveRepoPath(inputPath) {
  if (!inputPath || !inputPath.trim()) {
    fail('workflow_json_path is required.');
  }
  if (/^[a-z][a-z0-9+.-]*:/i.test(inputPath)) {
    fail('workflow_json_path must be a repository path, not a URL.');
  }
  if (path.isAbsolute(inputPath)) {
    fail('workflow_json_path must be relative to the repository root.');
  }
  const resolved = path.resolve(WORKSPACE, inputPath);
  if (!resolved.startsWith(WORKSPACE + path.sep) && resolved !== WORKSPACE) {
    fail('workflow_json_path escapes the repository root.');
  }
  if (!resolved.endsWith('.json')) {
    fail('workflow_json_path must point to a .json file.');
  }
  if (!fs.existsSync(resolved) || !fs.statSync(resolved).isFile()) {
    fail('workflow_json_path does not exist: ' + inputPath);
  }
  return resolved;
}
...
function readWorkflow(filePath) {
  ...
  if (/service[_-]?role/i.test(raw)) {
    fail('Workflow JSON appears to contain a service role reference. Remove secrets before importing.');
  }
  ...
  if (!Array.isArray(workflow.nodes)) {
    fail('Workflow JSON must include a nodes array.');
  }
...
function getWorkflowId(response) {
...
async function main() {
  ...
  const fallbackName = path.basename(workflowPath, '.json') + ' DEV import';
  ...
  if (!workflowId) {
    fail('n8n API did not return a workflow id after import.', { response: created });
  }
  ...
  console.log('::notice::n8n DEV workflow imported as inactive. Workflow id: ' + workflowId);
}

main().catch((error) => {
  fail('Unexpected n8n import failure.', { error: redact(error && error.stack ? error.stack : error) });
});

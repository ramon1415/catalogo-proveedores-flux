...
const companyEnvironments = [
  { environment: 'DEV', op: '9680353c-9b86-4730-82e1-fce664f048a2', fer: '68b61801-74c0-44ea-a33b-f20e4bf53aa7' },
  { environment: 'PROD', op: '144042c1-e493-4256-a86c-cd088a8898ce', fer: '20cd72aa-f281-4985-931b-a83422404b66' },
]
...
async function mountPage({ fetchBudget, fetchRequests, fetchActivity, pathname = '/dashboard', initialCompanyId = 'operadora' } = {}) {
  ...
  const { default: Page } = load(`${dashboardPath}DashboardPage.tsx`, {
    ...
    '../../lib/company': { useCompany: () => ({ companyId, companyName: companyId === 'operadora' || tenantConfig.usesDashboardLegacyIncome(companyId) ? 'Operadora QA' : 'Fersana QA' }) },
    ...
    './BudgetAccordion': load(`${dashboardPath}BudgetAccordion.tsx`, { './logic': logic, './budgetMovements': { fetchBudgetMovements: async () => [], movementTotals: () => ({ used: 0, executed: 0 }) }, './Dashboard.module.css': { __esModule: true, default: new Proxy({}, { get: (_, key) => String(key) }) }, './BudgetAccordion.module.css': { __esModule: true, default: new Proxy({}, { get: (_, key) => String(key) }) }, 'react-router-dom': { Link: span } }),
...
for (const { environment, op, fer } of companyEnvironments) test(`${environment}: activity API selects the income model per company and scopes all tenant data before pagination`, async () => {
...
for (const { environment, op, fer } of companyEnvironments) test(`${environment}: Fersana omits every incident surface and falls back to cash when switching from Operadora incidents`, async () => {
  const waiting = deferred()

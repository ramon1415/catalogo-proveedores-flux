...
test('actual page renders canonical budget split and neutral fiscal labels', async () => {
  ...
  try {
    ...
    // El resumen aprobado permanece visible incluso con el detalle cerrado.
...
test('unallocated categories remain in global consumption but never appear as category overruns', async () => {
  ...
  try {
    const dataRow = name => p.renderer.root.findAll(node => typeof node.props?.className === 'string' && node.props.className.split(' ')[0] === 'budgetRow' && text(node).includes(name))[0]
    const over = dataRow('Servicios QA'), unallocated = dataRow('Sin partida')
    assert.match(text(over), /12\.9% por encima/)
    assert.ok(over.props.className.includes('alert'))
    assert.match(text(over), /Excedente\$129/)
...
test('budget delta handles small overruns and exact budget without a false zero-percent overrun', async () => {
  // El acordeón expresa el excedente sobre presupuesto y conserva sobregiros mínimos.
  for (const { used, pctLabel, over } of [
    { used: 1039, pctLabel: '3.9% por encima', over: true },
    { used: 1000.01, pctLabel: '<0.1% por encima', over: true },
    ...
    try {
      const row = p.renderer.root.findAll(node => typeof node.props?.className === 'string' && node.props.className.split(' ')[0] === 'budgetRow')[0]

...
const company=id(1), actor=id(2), approver=id(3), provider=id(4), cc=id(5), ordinary=id(6), shared=id(7), other=id(8)
...
async function fixture() {
  ...
  await db.exec(`
    ...
    create function public.verify_budget_availability(uuid,uuid,uuid,date,numeric,boolean) returns jsonb language sql as $$select jsonb_build_object('status',case when $5<=100 then 'aprobable' else 'bloqueado' end)$$;
...
const rpc = (db, lines) => db.query(`select public.create_payment_request(
...
const line=(category,amount)=>({budget_category_id:category,cost_center_id:cc,amount})

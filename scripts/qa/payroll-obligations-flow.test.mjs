...
test('event insertion wakes only obligation notifications for an enabled company',async()=>{
 ...
 test('shared budget rejects a stale normal request and permits decreases and unrelated scopes',async()=>{
 const available=Number((await db.query('select available from budget_availability')).rows[0].available);
 await assert.rejects(db.query("insert into payment_requests(company_id,cost_center_id,budget_category_id,budget_month,amount_requested,status) values($1,$2,$3,'2026-07-01',$4,'submitted')",[company,center,category,available+1]),/presupuesto cambió/);
 const request=id(700);
 await db.query("insert into payment_requests(id,company_id,cost_center_id,budget_category_id,budget_month,amount_requested,status) values($1,$2,$3,$4,'2026-07-01',1,'submitted')",[request,company,center,category]);
 await db.query("update payment_requests set amount_requested=0.5 where id=$1",[request]);
 await db.query("update payment_requests set status='paid' where id=$1",[request]);

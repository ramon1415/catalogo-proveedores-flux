...
test("PROD forward migration is present and replaces the DEV ledger filename", () => {
  assert.equal(
    fs.existsSync(
      path.join(
        ROOT,
  ...
  assert.equal(
    fs.existsSync(
      path.join(
        ROOT,
        "supabase/migrations/00000000000000_payment_request_exception_quick_approve_prod.sql",
      ),
    ),
    false,
  );

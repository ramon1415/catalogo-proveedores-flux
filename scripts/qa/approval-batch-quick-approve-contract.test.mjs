...
function runtime(fetchMock) {
  return {
    env: (name) => ({
      ...
      SUPABASE_URL: "https://ucantptjhwttexzmslvm.supabase.co",
...
test("migration preserves the UI RPC and enforces one-time, all-pending and trigger-owned notification", async () => {
  const sql = await readFile(
    new URL("../../supabase/migrations/20260826225903_approval_batch_quick_approve_prod.sql", import.meta.url),
    "utf8",
  );
  assert.match(sql, /create or replace function public\.approve_entire_batch\(p_batch_id uuid\)/i);
  assert.match(sql, /approval_batch_require_actor\(\)/i);
  ...
  assert.match(sql, /https:\/\/flux\.quantta\.mx\/approval_batches\.html\?batch_id=/i);
  assert.doesNotMatch(sql, /scsirgbuqjcwoaxfacth|catalogo-proveedores-flux-git-dev/i);
});

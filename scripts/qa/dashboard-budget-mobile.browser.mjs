...
`, { flag: 'wx' })
  ...
  // Regressions for the two specificity collisions in the new cards.
  const alertsPage = await browser.newPage()
  const tokens = await readFile(path.join(app, 'src/theme/tokens.css'), 'utf8')
  const dashboardCss = await readFile(path.join(app, 'src/features/dashboard/Dashboard.module.css'), 'utf8')
  await alertsPage.setContent(`<style>${tokens}${dashboardCss}</style><main class="dash"><div class="budgetHeroSaldo"><strong class="alertText">-$10,000</strong></div><span class="reqSummarySep alertText">2 vencidos</span><span id="alert-control" class="alertText">Alerta</span></main>`)
  const alertColors = await alertsPage.evaluate(() => ['.budgetHeroSaldo strong', '.reqSummarySep', '#alert-control'].map(selector => getComputedStyle(document.querySelector(selector)).color))
  assert.equal(alertColors[0], alertColors[2]); assert.equal(alertColors[1], alertColors[2])
  console.log('PASS overrun and overdue summary retain the alert color')

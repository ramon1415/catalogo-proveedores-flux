...
test('detalle distingue el documento de origen del comprobante bancario final', () => {
  ...
  assert.ok(detail.includes('Adjuntar documento (opcional)'))
...
test('editar permite conservar una solicitud sin documento', () => {
  assert.ok(!editModal.includes('!request.invoice_storage_path && !file'))
  assert.ok(editModal.includes("'(reemplazo opcional)' : '(opcional)'"))
})

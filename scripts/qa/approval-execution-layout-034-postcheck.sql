-- Run only in Supabase DEV project scsirgbuqjcwoaxfacth after the single
-- successful execution of migration 034. Compare manifests with the private
-- PRECHECK_034 backup before declaring MIGRATION_034_POSTCHECK_PASS.
begin;
set transaction read only;

do $$
declare
  v_function_oid regprocedure;
  v_function_source text;
  v_function_config text[];
begin
  if to_regclass('public.company_directors_one_active_per_company_uidx')
       is not null then
    raise exception 'POSTCHECK_034_FAIL: one-active-per-company index remains';
  end if;

  if to_regclass('public.company_directors_active_uidx') is null then
    raise exception 'POSTCHECK_034_FAIL: active company/Director pair index is missing';
  end if;

  if to_regprocedure('public.add_company_director_for_future_batches(uuid,uuid)') is null
     or to_regprocedure('public.remove_company_director_for_future_batches(uuid,uuid)') is null
     or to_regprocedure('public.approval_batch_require_active_direction()') is null then
    raise exception 'POSTCHECK_034_FAIL: required 034 functions are missing';
  end if;

  if to_regprocedure('public.set_company_director_for_future_batches(uuid,uuid)')
       is not null then
    raise exception 'POSTCHECK_034_FAIL: replacement RPC still exists';
  end if;

  if exists (
    select 1
    from public.company_directors director_assignment
    where director_assignment.active
    group by
      director_assignment.company_id,
      director_assignment.director_profile_id
    having count(*) > 1
  ) then
    raise exception 'POSTCHECK_034_FAIL: duplicate active company/Director pairs exist';
  end if;

  if exists (
    select 1
    from public.approval_batches batch
    where batch.director_id is null
  ) then
    raise exception 'POSTCHECK_034_FAIL: a batch lost its Director snapshot';
  end if;

  foreach v_function_oid in array array[
    'public.add_company_director_for_future_batches(uuid,uuid)'::regprocedure,
    'public.remove_company_director_for_future_batches(uuid,uuid)'::regprocedure,
    'public.create_approval_batch(uuid,text,date,date,uuid,text)'::regprocedure,
    'public.list_director_approval_batches(text)'::regprocedure,
    'public.approve_entire_batch(uuid)'::regprocedure,
    'public.decide_approval_batch_items(uuid,jsonb)'::regprocedure
  ]
  loop
    select
      pg_get_functiondef(procedure.oid),
      procedure.proconfig
      into strict v_function_source, v_function_config
    from pg_proc procedure
    where procedure.oid = v_function_oid::oid
      and procedure.prosecdef;

    if not coalesce(v_function_config, array[]::text[])
      @> array['search_path=public, pg_temp']::text[] then
      raise exception 'POSTCHECK_034_FAIL: fixed search_path missing: %',
        v_function_oid;
    end if;

    if not has_function_privilege(
      'authenticated',
      v_function_oid::text,
      'EXECUTE'
    )
       or has_function_privilege('anon', v_function_oid::text, 'EXECUTE') then
      raise exception 'POSTCHECK_034_FAIL: RPC ACL drift: %', v_function_oid;
    end if;
  end loop;

  select lower(pg_get_functiondef(
    'public.create_approval_batch(uuid,text,date,date,uuid,text)'::regprocedure
  ))
    into v_function_source;

  if position('profile_company_memberships' in v_function_source) = 0
     or position('director_assignment.active' in v_function_source) = 0
     or position('director_profile.active' in v_function_source) = 0 then
    raise exception 'POSTCHECK_034_FAIL: create-batch eligibility is incomplete';
  end if;

  foreach v_function_oid in array array[
    'public.list_director_approval_batches(text)'::regprocedure,
    'public.approve_entire_batch(uuid)'::regprocedure,
    'public.decide_approval_batch_items(uuid,jsonb)'::regprocedure
  ]
  loop
    select lower(pg_get_functiondef(v_function_oid))
      into v_function_source;

    if position(
      'approval_batch_require_active_direction' in v_function_source
    ) = 0
       or position('company_directors' in v_function_source) > 0 then
      raise exception 'POSTCHECK_034_FAIL: batch snapshot authorization drift: %',
        v_function_oid;
    end if;
  end loop;
end
$$;

with target_company as (
  select company.id
  from public.companies company
  where lower(
    coalesce(nullif(btrim(company.legal_name), ''), company.name)
  ) like '%operadora tlacatecpan%'
),
active_directors as (
  select
    director_assignment.director_profile_id,
    profile.full_name
  from public.company_directors director_assignment
  join target_company company on company.id = director_assignment.company_id
  join public.profiles profile
    on profile.id = director_assignment.director_profile_id
  where director_assignment.active
),
denise_assignments as (
  select director_assignment.active
  from public.company_directors director_assignment
  join target_company company on company.id = director_assignment.company_id
  join public.profiles profile
    on profile.id = director_assignment.director_profile_id
  where translate(
    lower(coalesce(profile.full_name, '')),
    'áéíóúüñ',
    'aeiouun'
  ) like 'denise%'
)
select
  'POSTCHECK_034_DIRECTOR_RECONCILIATION' as check_name,
  (select count(*) from target_company) as target_company_count,
  (select count(*) from active_directors) as active_director_count,
  (
    select count(*)
    from active_directors director
    where translate(
      lower(coalesce(director.full_name, '')),
      'áéíóúüñ',
      'aeiouun'
    ) like 'ramon%'
  ) as active_ramon_count,
  (select count(*) from denise_assignments where active) as active_denise_count,
  (select count(*) from denise_assignments where not active) as inactive_denise_count;

select
  'POSTCHECK_034_OBJECTS' as check_name,
  'scsirgbuqjcwoaxfacth'::text as expected_dev_project_ref,
  to_regclass('public.company_directors_one_active_per_company_uidx')
    is null as one_active_index_removed,
  to_regclass('public.company_directors_active_uidx')
    is not null as active_pair_index_present,
  to_regprocedure('public.add_company_director_for_future_batches(uuid,uuid)')
    is not null as add_rpc_present,
  to_regprocedure('public.remove_company_director_for_future_batches(uuid,uuid)')
    is not null as remove_rpc_present,
  to_regprocedure('public.set_company_director_for_future_batches(uuid,uuid)')
    is null as replacement_rpc_removed;

select
  'POSTCHECK_034_MANIFEST' as check_name,
  (select count(*) from public.approval_batches) as approval_batches_count,
  (
    select md5(coalesce(string_agg(
      concat_ws(
        '|',
        batch.id,
        batch.company_id,
        batch.director_id,
        batch.status,
        batch.created_by,
        batch.submitted_by,
        batch.decided_by,
        batch.closed_by,
        batch.created_at,
        batch.submitted_at,
        batch.decided_at,
        batch.closed_at
      ),
      E'\n'
      order by batch.id
    ), ''))
    from public.approval_batches batch
  ) as approval_batches_manifest,
  (select count(*) from public.approval_batch_items) as approval_batch_items_count,
  (
    select md5(coalesce(string_agg(
      concat_ws(
        '|',
        batch_item.id,
        batch_item.batch_id,
        batch_item.payment_request_id,
        batch_item.director_status,
        batch_item.decided_by,
        batch_item.decided_at,
        batch_item.removed_at
      ),
      E'\n'
      order by batch_item.id
    ), ''))
    from public.approval_batch_items batch_item
  ) as approval_batch_items_manifest,
  (select count(*) from public.payment_receipts) as payment_receipts_count,
  (
    select md5(coalesce(string_agg(
      concat_ws(
        '|',
        receipt.id,
        receipt.payment_request_id,
        receipt.created_at
      ),
      E'\n'
      order by receipt.id
    ), ''))
    from public.payment_receipts receipt
  ) as payment_receipts_manifest,
  (select count(*) from public.approval_batch_company_settings)
    as enforcement_settings_count,
  (
    select md5(coalesce(string_agg(
      concat_ws(
        '|',
        setting.company_id,
        setting.regular_payments_require_closed_batch,
        setting.enforcement_started_at,
        setting.updated_at
      ),
      E'\n'
      order by setting.company_id
    ), ''))
    from public.approval_batch_company_settings setting
  ) as enforcement_settings_manifest;

rollback;

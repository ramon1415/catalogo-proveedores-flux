...
const migration = read('supabase/migrations/20260924171908_dashboard_budget_movements.sql')
...
async function fixture() {
 ...
 create table public.historical_actuals_periods(company_id uuid,period_month date,status text);
 create table private.dashboard_historical_spend_v2(company_id uuid,period_month date,budget_category_id uuid,classification text,historical_executed numeric,account_count int);
 ...
 insert into historical_actuals_periods values('${company}','2026-08-01','certified');
 insert into private.dashboard_historical_spend_v2 values
 ('${company}','2026-08-01','${company}','partida',300,2),
 ('${company}','2026-08-01',null,'por_clasificar',70,1),
 ('${company}','2026-08-01',null,'sin_partida',80,1);
 set test.actor='${company}'; set test.role='director';
 `)
 const canonical=read('supabase/migrations/20260922151518_dashboard_historical_blend_v2.sql')
 await db.exec(canonical.slice(canonical.indexOf('create or replace function private.dashboard_global_budget_report_v2'),canonical.lastIndexOf('commit;')))
...
test('detail reconciles with canonical summary: subtotal, FX, exceptions, payroll; rejects excluded rows and other companies',async()=>{
 ...
 const summary=(await db.query(`select public.dashboard_global_budget_report_v2('${company}',2026) as rows`)).rows[0].rows
...
test('certified history replaces operational requests and IMSS exactly once, including annual and unclassified scope',async()=>{
 const db=await fixture();try {
 const rows=await detail(db,company,"'2026-08-01'");assert.equal(rows.length,1);assert.equal(rows[0].source,'historical');assert.equal(Number(rows[0].amount),300)
 assert.equal((await detail(db,company,'null')).reduce((n,r)=>n+Number(r.amount),0),870)
 assert.equal(Number((await detail(db,'__por_clasificar__',"'2026-08-01'"))[0].amount),70)
 assert.equal(Number((await detail(db,'__sin_partida__',"'2026-08-01'"))[0].amount),80)
 }finally{await db.close()}
})

...
test('controlled aggregate seed is 98 + 62 and aborts on tenant identity mismatch', () => {
  ...
  assert.match(sql, /Adquisiciòn de herramientas' then 'Adquisición de herramientas/)
  assert.match(sql, /v\.category_name in \('Gastos extraordinarios', 'Servicios y suministros'\)/)
  assert.match(sql, /btrim\(bc\.name\) = btrim\(v_target_name\)/)
  assert.match(sql, /rc\.resolved_count = 0 then false/)
  assert.match(sql, /partida_prediction_resolved_candidate_invalid/)
  assert.match(sql, /partida_prediction_empty_candidate_still_confident/)
  assert.match(sql, /partida_prediction_stale_candidate_survived/)
})

-- Function definitions read from PROD before promotion; no user records.
...
CREATE OR REPLACE FUNCTION public.create_payment_request(p_proveedor_id uuid, p_company_id uuid, p_cost_center_id uuid, p_budget_category_id uuid, p_budget_month date, p_amount_requested numeric, p_currency text, p_exchange_rate numeric, p_description text, p_notes text, p_requested_by uuid, p_is_extraordinary_adjustment boolean, p_approver_id uuid, p_approver_assignment_id uuid, p_subtotal_amount numeric, p_tax_amount numeric, p_withholding_amount numeric, p_invoice_uuid text, p_beneficiary_profile_id uuid, p_request_type text, p_partida_unsure boolean)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO 'public', 'pg_temp'
AS $function$
...
begin
  ...
  if v_request_id is null then
    raise exception 'partida_unsure_payment_request_id_missing';
  end if;

  update public.payment_requests
     set partida_unsure = coalesce(p_partida_unsure, false),
         updated_at = now()
   where id = v_request_id;

  if not found then
    raise exception 'partida_unsure_link_failed';
  end if;
...
CREATE OR REPLACE FUNCTION public.create_payment_request_with_document(p_proveedor_id uuid, p_company_id uuid, p_cost_center_id uuid, p_budget_category_id uuid, p_budget_month date, p_amount_requested numeric, p_currency text DEFAULT 'MXN'::text, p_exchange_rate numeric DEFAULT 1, p_description text DEFAULT NULL::text, p_notes text DEFAULT NULL::text, p_requested_by uuid DEFAULT NULL::uuid, p_is_extraordinary_adjustment boolean DEFAULT false, p_approver_id uuid DEFAULT NULL::uuid, p_approver_assignment_id uuid DEFAULT NULL::uuid, p_subtotal_amount numeric DEFAULT NULL::numeric, p_tax_amount numeric DEFAULT NULL::numeric, p_withholding_amount numeric DEFAULT NULL::numeric, p_invoice_uuid text DEFAULT NULL::text, p_beneficiary_profile_id uuid DEFAULT NULL::uuid, p_request_type text DEFAULT NULL::text, p_invoice_storage_path text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.create_payment_request_with_document(p_proveedor_id uuid, p_company_id uuid, p_cost_center_id uuid, p_budget_category_id uuid, p_budget_month date, p_amount_requested numeric, p_currency text, p_exchange_rate numeric, p_description text, p_notes text, p_requested_by uuid, p_is_extraordinary_adjustment boolean, p_approver_id uuid, p_approver_assignment_id uuid, p_subtotal_amount numeric, p_tax_amount numeric, p_withholding_amount numeric, p_invoice_uuid text, p_beneficiary_profile_id uuid, p_request_type text, p_invoice_storage_path text, p_partida_unsure boolean)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO 'public', 'pg_temp'
AS $function$
...
begin
  ...
  if v_request_id is null then
    raise exception 'partida_unsure_document_request_id_missing';
  end if;
  ...
  if not found then
    raise exception 'partida_unsure_document_link_failed';
  end if;
...
CREATE OR REPLACE FUNCTION public.verify_budget_availability(p_company_id uuid, p_cost_center_id uuid, p_budget_category_id uuid, p_budget_month date, p_amount numeric, p_is_extraordinary_adjustment boolean, p_no_presupuestal boolean)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO 'public', 'pg_temp'
AS $function$
...
CREATE OR REPLACE FUNCTION public.decide_payment_request_internal(p_payment_request_id uuid, p_actor_profile_id uuid, p_action text, p_comments text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'

...
export function validateDocument(doc:Document) {
  ...
  for(const r of doc.rows) {
    if(!r.id || ids.has(r.id) || !labels[r.status] || !Number.isSafeInteger(r.amount_minor) || r.amount_minor<0
...
export function renderEmail(doc:Document) {
  ...
  const state=`${approved} aprobada(s), programada(s) o pagada(s) · ${pending} pendiente(s)${cancelled?` · ${cancelled} cancelada(s)`:''}.`
...
export function renderPdf(doc:Document):Uint8Array {
  ...
  for(let i=0;i<groups.length;i++) {
    ...
    pdf.text(doc.environment==='dev'?'PRUEBA DEV - Reporte informativo, no autoriza pagos.':'Reporte informativo; incluye solicitudes pendientes de aprobación.',36,104)
  ...
  for(let p=1;p<=pdf.getNumberOfPages();p++){pdf.setPage(p);pdf.setFontSize(8);pdf.setTextColor(104,113,109);pdf.text('Flux - Corte semanal | Excluye rechazadas | No autoriza ni ejecuta pagos',36,590);pdf.text(`${p} / ${pdf.getNumberOfPages()}`,756,590,{align:'right'})}

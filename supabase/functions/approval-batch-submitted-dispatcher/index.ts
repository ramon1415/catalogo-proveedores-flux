...
export function generateApprovalBatchPdfBytes(document, logoBytes = null) {
  ...
  if (logoBytes?.length) {
    try {
      ...
      // Preserve the same system behavior: the PDF remains valid if the logo cannot be decoded.
    }
  }
...
export async function prepareApprovalBatchAttachment(document, runtime = { fetch: globalThis.fetch.bind(globalThis) }) {
...
export function renderApprovalBatchSubmittedEmail(document, sendMode, deliveryMode = "test_recipient") {
  ...
  const text = [
    ...
    "El PDF adjunto es el mismo formato disponible en el botón PDF del corte.", "",
...
export async function handleRequest(req, runtime) {
  ...
  try {
    ...
    if (sendMode === "disabled") return jsonResponse({ processed: 0, sent: 0, failed: 0, cancelled: 0, mode: sendMode, delivery_mode: deliveryMode, event_type: EVENT_TYPE, message: `${FUNCTION_NAME} disabled; no events claimed` });

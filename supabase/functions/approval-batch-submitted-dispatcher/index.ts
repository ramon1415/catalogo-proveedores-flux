...
const EVENT_TYPE = "approval_batch.submitted" as const;
const FUNCTION_NAME = "approval-batch-submitted-dispatcher";
const DEFAULT_WORKER_ID = "edge-approval-batch-submitted-prod";
...
function totalsText(value: unknown): string {
...
function wrap(value: unknown, maxChars: number, maxLines = 2): string[] {
  const text = ascii(value);
  if (!text) return ["-"];
  ...
  if (text.length > consumed && lines.length) {
...
function pdfPageContent(document: ApprovalBatchDocument, pageIndex: number, pageCount: number, pageItems: ApprovalBatchItem[]): string {
  ...
  const headers = ["#", "Folio", "Proveedor", "Concepto", "Centro / Partida", "Importe"];
  const headerX = [42, 65, 170, 345, 505, 630];
  headers.forEach((header, index) => {
    out += textCommand("F2", 8.5, headerX[index], 404, header, "0.086 0.196 0.176");
  });
...
export function generateApprovalBatchPdfBytes(document: ApprovalBatchDocument): { bytes: Uint8Array; pageCount: number } {
  const items = Array.isArray(document.items) ? document.items : [];
  const rowsPerPage = PDF_ROWS_PER_PAGE;
  const pageCount = Math.max(Math.ceil(items.length / rowsPerPage), 1);
...
export async function prepareApprovalBatchAttachment(document: ApprovalBatchDocument): Promise<PreparedAttachment> {
...
export function renderApprovalBatchSubmittedEmail(
  document: ApprovalBatchDocument,
  sendMode: string,
      ...
      <tr>
        <td align="center" style="padding:24px 12px 18px;">
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" bgcolor="#ffffff" style="width:100%;max-width:560px;border:1px solid #d8ddd5;border-radius:14px;border-collapse:separate;overflow:hidden;background:#ffffff;">
            <tr>
              <td bgcolor="#16322d" style="padding:20px 28px;background:#16322d;"><img src="${EMAIL_LOGO_URL}" width="110" alt="Flux" style="display:block;width:110px;max-width:100%;height:auto;border:0;" /></td>
            </tr>
            <tr>
              <td style="padding:24px 28px 30px;font-family:Arial,Helvetica,sans-serif;color:#1f2926;">
                <h1 style="margin:0 0 12px;font-family:Georgia,'Times New Roman',serif;font-size:24px;line-height:1.2;color:#16322d;">Tienes un corte por autorizar</h1>
                <p style="margin:0 0 10px;font-size:14px;line-height:1.5;color:#1f2926;">Finanzas envió un corte semanal que requiere tu revisión y decisión en Flux.</p>
                <p style="margin:0 0 16px;font-size:14px;line-height:1.5;color:#1f2926;">El PDF adjunto contiene el detalle de los pagos incluidos.</p>
                ...
                <table role="presentation" cellspacing="0" cellpadding="0" border="0" style="margin-top:22px;">
                  <tr>
                    <td bgcolor="#16322d" style="border-radius:6px;">
                      <a href="${targetUrl}" style="display:inline-block;padding:11px 18px;color:#ffffff;font-family:Arial,sans-serif;font-size:14px;font-weight:700;text-decoration:none;">Revisar y autorizar corte</a>
                    </td>
                  </tr>
                </table>
                ${testBanner}
...
async function callRpc<T>(
  ...
  if (!response.ok) {
    ...
    throw new Error(`rpc_${rpcName}_failed:${response.status}:${message}`);
  }
...
export async function readDispatchOptions(req: Request, runtime: Runtime): Promise<DispatchOptions> {
  ...
  const cutoffValue = String(
    body.created_at_after ?? optionalEnv(runtime, "APPROVAL_BATCH_SUBMITTED_CUTOFF_AT"),
  ).trim();
...
function isStaleBatchError(error: unknown): boolean {
  const message = error instanceof Error ? error.message : String(error || "");
  return message.includes("approval_batch_no_longer_submitted") ||
    message.includes("approval_batch_notification_recipient_drift");
}
...
export async function handleRequest(req: Request, runtime: Runtime): Promise<Response> {
  ...
  try {
    ...
    if (providedSecret !== expectedSecret) return jsonResponse({ error: "unauthorized" }, 401);
    ...
    if (sendMode === "disabled") {
      return jsonResponse({
        ...
        message: `${FUNCTION_NAME} disabled; no events claimed`,
      });
    }
    ...
    const events = await callRpc<NotificationEvent[]>(
      ...
      supabaseUrl,
      serviceRoleKey,
      "claim_approval_batch_submitted_events_for_dispatcher",
      {
    ...
    for (const event of events) {
      ...
      try {
        ...
        const document = await callRpc<ApprovalBatchDocument>(
          runtime.fetch,
          supabaseUrl,
          serviceRoleKey,
        ...
        attachment = await prepareApprovalBatchAttachment(document);
        ...
        results.push({
          ...
          attachment_pages: attachment.pageCount,
        });
      } catch (error) {
        const message = error instanceof Error ? error.message : "approval_batch_submitted_dispatch_failed";
        if (isStaleBatchError(error)) {
          try {
            await callRpc(runtime.fetch, supabaseUrl, serviceRoleKey, "cancel_approval_batch_submitted_event_for_dispatcher", {
              p_event_id: event.id,
              p_worker_id: workerId,
              p_reason: "approval_batch_no_longer_submitted",
            });
            cancelled += 1;
            results.push({
              event_id: event.id,
            ...
            // Fall through to the standard failure ledger if cancellation cannot be recorded.
          }
        }

...
function totalsText(value) {
  ...
  return value
    .map((row) => row && typeof row === "object" ? formatMoney(row.amount, row.currency) : null)
    .filter(Boolean)
    .join(" | ");
}
...
function systemPdfRows(document) {
...
export function generateApprovalBatchPdfBytes(document, logoBytes = null) {
  if (typeof autoTable !== "function") throw new Error("jspdf_autotable_adapter_unavailable");
  ...
  if (logoBytes?.length) {
    try {
      ...
      // Same fallback as the frontend: keep the PDF valid if the logo cannot be decoded.
    }
  }
  ...
  doc.text(
    `${batch.company_name || batch.company || "-"} | ${batch.period_start || "-"} a ${batch.period_end || "-"} | ${statusLabel(batch.status)}`,
    36,
    53,
  );
...
export async function prepareApprovalBatchAttachment(
  document,
  runtime = { fetch: globalThis.fetch.bind(globalThis) },
) {
  const logoBytes = await fetchSystemPdfLogo(runtime.fetch);
  const { bytes, pageCount } = generateApprovalBatchPdfBytes(document, logoBytes);
  ...
  if (!signature.startsWith("%PDF-1.")) throw new Error("approval_batch_pdf_signature_invalid");
  return {
...
export function renderApprovalBatchSubmittedEmail(
  ...
  const summaryRows = [
    ["Corte", batch.label],
    ["Empresa", batch.company_name || batch.company],
...
export async function handleRequest(req, runtime) {
  ...
  try {
    ...
    if ((req.headers.get("x-notification-dispatcher-secret") || "") !== expectedSecret) {
    ...
    if (!["director", "test_recipient"].includes(deliveryMode)) {
      return jsonResponse({ error: "invalid_approval_batch_submitted_delivery_mode" }, 500);
    }
    if (sendMode === "disabled") {
      return jsonResponse({
        processed: 0, sent: 0, failed: 0, cancelled: 0,
        mode: sendMode, delivery_mode: deliveryMode, event_type: EVENT_TYPE,
    ...
    const events = await callRpc(
      ...
      { p_limit: options.limit, p_worker_id: workerId, p_created_at_after: options.createdAtAfter },
    );
      ...
      try {
        ...
        if (isStaleBatchError(error)) {
          try {
            ...
            // Fall through to the standard failure ledger.
          }
        }
        ...
        try {
          ...
          markError = markFailure instanceof Error
            ? `;mark_failed_error:${markFailure.message}`
            : ";mark_failed_error:unknown";
        }
        failed += 1;
        results.push({
          event_id: event.id,

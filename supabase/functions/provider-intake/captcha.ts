...
export class TurnstileVerifier implements CaptchaVerifier {
  ...
  constructor(options: TurnstileOptions) {
    if (!options.secret.trim()) {
      throw new Error("missing_required_secret:CAPTCHA_SECRET");
    }
  ...
  async verify(input: CaptchaVerificationInput): Promise<boolean> {
    ...
    if (
      this.options.expectedHostname &&
      result.hostname !== this.options.expectedHostname
    ) return false;
    if (
      this.options.expectedAction &&
    ...
    if (
      !Number.isFinite(challengedAt) || challengedAt > this.now() + 60000 ||
      this.now() - challengedAt > maxAgeMs
    ) {
      return false;
    }
    return true;

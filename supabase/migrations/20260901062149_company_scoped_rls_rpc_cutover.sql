...
drop policy if exists payment_request_cfdi_facts_select
  on public.payment_request_cfdi_facts;
create policy payment_request_cfdi_facts_select
on public.payment_request_cfdi_facts for select to authenticated
using (
  exists (
    select 1
...
drop policy if exists payment_request_cfdi_facts_insert
  on public.payment_request_cfdi_facts;
create policy payment_request_cfdi_facts_insert
on public.payment_request_cfdi_facts for insert to authenticated
with check (
...
drop policy if exists payroll_provision_settings_finance_read
  on public.payroll_provision_settings;
create policy payroll_provision_settings_finance_read
on public.payroll_provision_settings for select to authenticated
using ((select private.current_profile_has_company_role(
  company_id,
  array['finance']::text[]
)));

drop policy if exists payroll_provision_entries_finance_read
  on public.payroll_provision_entries;
create policy payroll_provision_entries_finance_read
on public.payroll_provision_entries for select to authenticated
using ((select private.current_profile_has_company_role(
  company_id,
  array['finance']::text[]
)));

drop policy if exists payroll_contpaq_role_mappings_finance_read
  on public.payroll_contpaq_role_mappings;
create policy payroll_contpaq_role_mappings_finance_read
on public.payroll_contpaq_role_mappings for select to authenticated
using ((select private.current_profile_has_company_role(
  company_id,
  array['finance']::text[]
)));

drop policy if exists payroll_contpaq_bank_mappings_finance_read
  on public.payroll_contpaq_bank_mappings;
create policy payroll_contpaq_bank_mappings_finance_read
on public.payroll_contpaq_bank_mappings for select to authenticated
using ((select private.current_profile_has_company_role(
  company_id,
  array['finance']::text[]
)));

...
create or replace function public.claim_approval_batch_decision_events_for_dispatcher(
  ...
  p_worker_id text default 'edge-notification-dispatcher-dev',
...
declare
  ...
  v_worker_id text := left(
    coalesce(nullif(btrim(p_worker_id), ''), 'edge-notification-dispatcher-dev'),
    120
  );
begin
...
comment on function public.claim_approval_batch_decision_events_for_dispatcher(
  ...
  'Claims only final weekly-cut decisions strictly newer than the DEV activation cutoff and only for the active submitted_by recipient.';

comment on function public.notification_approval_batch_decision_dispatch_wakeup_internal() is
  'Best-effort post-commit DEV wake-up for new final weekly-cut decision events using an independent immutable cutoff.';

...
begin
  ...
  or exists (
    ...
    raise exception '037_precheck: migration 036 distribution is not 7/1/1/0';
  end if;
end
$precheck$;
...
alter table public.payment_request_extraordinary_authorizations
  ...
  check (
    ...
    or (
      ...
      and evidence_storage_bucket = 'extraordinary-approval-evidence'
...
-- Post-Yanin mixed-close correction. Direction's decision remains immutable;
...
begin
  ...
  if not v_policy.evidence_required then
    raise exception 'extraordinary_evidence_policy_required';
  end if;
  if v_request.currency <> 'MXN'
     or v_request.amount_requested > v_policy.max_amount_mxn then
  ...
  return jsonb_build_object(
    ...
    'status', 'draft',
    'storage_bucket', 'extraordinary-approval-evidence',
    'storage_path', v_path,
...
to authenticated
with check (
  bucket_id = 'extraordinary-approval-evidence'

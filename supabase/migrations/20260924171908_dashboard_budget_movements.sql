-- Read-only drilldown; same access checks, consumption and historical replacement
-- as dashboard_global_budget_report_v2. No changes to payments or budget gates.
begin;
...
begin
  ...
  with certified as (
    select period_month from public.historical_actuals_periods
    where company_id=p_company_id and status='certified' and period_month >= v_start and period_month < v_end
  ), requests as (
    ...
    where pr.company_id=p_company_id and pr.budget_month>=v_start and pr.budget_month<v_end
      ...
      and not exists(select 1 from certified c where c.period_month=pr.budget_month)
    ...
    where o.company_id=p_company_id and o.budget_month>=v_start and o.budget_month<v_end
      ...
      and not exists(select 1 from certified c where c.period_month=o.budget_month)
    union all
    select 'historical-'||h.period_month::text, 'historical', 'CONTPAQ', 'Histórico certificado',
      h.account_count::text||' cuentas · consolidado mensual', h.period_month,h.period_month,'paid',h.historical_executed
    from private.dashboard_historical_spend_v2 h join certified c on c.period_month=h.period_month
    where h.company_id=p_company_id and h.budget_category_id is not distinct from v_category
      and h.classification=case p_category_key when '__por_clasificar__' then 'por_clasificar' when '__sin_partida__' then 'sin_partida' else 'partida' end
  )

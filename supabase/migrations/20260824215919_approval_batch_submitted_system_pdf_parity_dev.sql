...
declare
  ...
  v_worker_id text := left(
    coalesce(nullif(btrim(p_worker_id), ''), 'edge-approval-batch-submitted-dev'),
    120
  );
begin
  select *
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'payment_method', coalesce(
      ...
      case
        when request.request_type::text in ('cash', 'check') then request.request_type::text
        else 'transfer'
      end
    ),
    'amount', request.amount_requested,
    'currency', coalesce(nullif(upper(btrim(request.currency)), ''), 'MXN'),
    'requester_name', coalesce(nullif(btrim(requester.full_name), ''), requester.email, '-'),
...
comment on function public.get_approval_batch_submitted_notification_document(uuid, text)
  is 'Returns the selected-Director submitted batch snapshot using the same field contract as the approval_batches system PDF export.';

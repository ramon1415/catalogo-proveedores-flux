...
begin
  ...
  if v_total <> 9 or v_active <> 8 or v_revoked <> 1 then
    raise exception
      '036_precheck: legacy authorization baseline mismatch (total %, active %, revoked %)',
  ...
  if v_consumed <> 7 or v_quarantined <> 1 then
    raise exception
      '036_precheck: direct lineage distribution mismatch (consumed %, quarantined %, revoked %)',
      v_consumed, v_quarantined, v_revoked;
  end if;
end
$precheck$;
...
begin
  ...
  if v_consumed <> 7 or v_quarantined <> 1 or v_revoked <> 1 or v_active <> 0 then
    raise exception
      '036_postcheck: expected 7/1/1/0, got %/%/%/%',

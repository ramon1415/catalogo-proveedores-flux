...
begin
  ...
  if to_regprocedure('public.claim_approval_batch_submitted_events_for_dispatcher(integer,text,timestamptz)') is not null
     or to_regprocedure('public.get_approval_batch_submitted_notification_document(uuid,text)') is not null
     or to_regprocedure('public.cancel_approval_batch_submitted_event_for_dispatcher(uuid,text,text)') is not null
     or to_regprocedure('public.notification_approval_batch_submitted_dispatch_wakeup_internal()') is not null
     or exists (
       select 1
       from pg_trigger trigger
...
create function public.claim_approval_batch_submitted_events_for_dispatcher(
  ...
  p_worker_id text default 'edge-approval-batch-submitted-dev',
...
begin
  ...
  with candidate as (
    ...
    order by case event.priority when 'critical' then 1 when 'high' then 2 when 'normal' then 3 when 'low' then 4 else 5 end,
      event.created_at,
      event.id
  ...
  claimed as (
    update public.notification_events event
     ...
     returning event.id, event.event_type, event.source_table, event.source_id, event.source_folio,
       event.recipient_type, event.recipient_profile_id, event.recipient_email, event.channel,
       event.priority, event.subject, event.payload, event.attempt_count
  )
  select claimed.id, claimed.event_type, claimed.source_table, claimed.source_id, claimed.source_folio,
    claimed.recipient_type, claimed.recipient_profile_id, claimed.recipient_email, claimed.channel,
    claimed.priority, claimed.subject, claimed.payload, claimed.attempt_count
  from claimed
  order by claimed.id;
end;
$$;

alter function public.claim_approval_batch_submitted_events_for_dispatcher(integer, text, timestamp with time zone) owner to postgres;
revoke all on function public.claim_approval_batch_submitted_events_for_dispatcher(integer, text, timestamp with time zone) from public, anon, authenticated;
grant execute on function public.claim_approval_batch_submitted_events_for_dispatcher(integer, text, timestamp with time zone) to service_role;
...
declare
  ...
  v_worker_id text := left(coalesce(nullif(btrim(p_worker_id), ''), 'edge-approval-batch-submitted-dev'), 120);
begin
  select * into v_event from public.notification_events event where event.id = p_notification_event_id;
  if not found then raise exception 'approval_batch_notification_event_not_found'; end if;
  if v_event.event_type <> 'approval_batch.submitted' or v_event.source_table <> 'approval_batches' or v_event.source_id is null then
    raise exception 'approval_batch_notification_event_invalid';
  end if;
  if v_event.status <> 'processing' or nullif(btrim(coalesce(v_event.locked_by, '')), '') is null or v_event.locked_by <> v_worker_id then
    raise exception 'approval_batch_notification_event_not_claimed_by_worker';
  end if;

  select * into v_batch from public.approval_batches batch where batch.id = v_event.source_id;
  ...
  select * into v_director from public.profiles profile where profile.id = v_batch.director_id and coalesce(profile.active, true);
  if not found
  ...
  if v_item_count < 1 then raise exception 'approval_batch_notification_requires_items'; end if;
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'provider', coalesce(nullif(btrim(provider.alias), ''), nullif(btrim(provider.nombre_completo), ''), 'Proveedor sin nombre'),
    'concept', coalesce(nullif(btrim(request.payment_concept), ''), nullif(btrim(request.concept), ''), nullif(btrim(request.description), ''), 'Sin concepto'),
    'cost_center', case when cost_center.id is null then null else concat_ws(' - ', nullif(btrim(cost_center.code), ''), nullif(btrim(cost_center.name), '')) end,
    'budget_category', case when budget_category.id is null then null else concat_ws(' - ', nullif(btrim(budget_category.code), ''), nullif(btrim(budget_category.name), '')) end,
    'payment_method', coalesce(nullif(btrim(request.payment_method), ''), nullif(btrim(request.request_type::text), ''), 'otro'),
  ...
  left join public.proveedores provider on provider.id = request.proveedor_id
  left join public.cost_centers cost_center on cost_center.id = request.cost_center_id
  left join public.budget_categories budget_category on budget_category.id = request.budget_category_id
  where item.batch_id = v_batch.id and item.removed_at is null;
...
alter function public.get_approval_batch_submitted_notification_document(uuid, text) owner to postgres;
revoke all on function public.get_approval_batch_submitted_notification_document(uuid, text) from public, anon, authenticated;
grant execute on function public.get_approval_batch_submitted_notification_document(uuid, text) to service_role;
...
declare
  v_event public.notification_events%rowtype;
  v_worker_id text := left(coalesce(nullif(btrim(p_worker_id), ''), 'edge-approval-batch-submitted-dev'), 120);
  v_reason text := left(coalesce(nullif(btrim(p_reason), ''), 'approval_batch_no_longer_submitted'), 300);
begin
  select * into v_event from public.notification_events event where event.id = p_event_id for update;
  if not found then raise exception 'approval_batch_notification_event_not_found'; end if;
  if v_event.event_type <> 'approval_batch.submitted' then raise exception 'approval_batch_notification_event_invalid'; end if;
  if v_event.status <> 'processing' or nullif(btrim(coalesce(v_event.locked_by, '')), '') is null or v_event.locked_by <> v_worker_id then
    raise exception 'approval_batch_notification_event_not_claimed_by_worker';
  end if;

  update public.notification_events
     set status = 'cancelled', last_error = v_reason, next_attempt_at = null,
         locked_at = null, locked_by = null, updated_at = now()
   where id = p_event_id;

  return jsonb_build_object('event_id', p_event_id, 'status', 'cancelled', 'reason', v_reason);
end;
$$;

alter function public.cancel_approval_batch_submitted_event_for_dispatcher(uuid, text, text) owner to postgres;
revoke all on function public.cancel_approval_batch_submitted_event_for_dispatcher(uuid, text, text) from public, anon, authenticated;
grant execute on function public.cancel_approval_batch_submitted_event_for_dispatcher(uuid, text, text) to service_role;
...
declare
  ...
  v_request_id bigint;
begin
  if new.event_type <> 'approval_batch.submitted' or new.status <> 'pending' then return new; end if;

  begin
    select
      max(secret.decrypted_secret) filter (where secret.name = 'notification_approval_batch_submitted_dispatcher_url'),
      max(secret.decrypted_secret) filter (where secret.name = 'notification_dispatcher_secret'),
      max(secret.decrypted_secret) filter (where secret.name = 'notification_approval_batch_submitted_cutoff_at'),
      max(secret.decrypted_secret) filter (where secret.name = 'notification_approval_batch_submitted_immediate_enabled')
    ...
    if lower(coalesce(v_enabled, 'false')) <> 'true' then return new; end if;
    if nullif(btrim(v_url), '') is null or nullif(btrim(v_secret), '') is null or nullif(btrim(v_cutoff), '') is null then return new; end if;
    if v_url !~ '^https://[a-z0-9]{20}\.supabase\.co/functions/v1/approval-batch-submitted-dispatcher$' then return new; end if;

    v_cutoff_at := v_cutoff::timestamp with time zone;
    if new.created_at <= v_cutoff_at then return new; end if;

    select net.http_post(
      url := v_url,
      body := jsonb_build_object('created_at_after', v_cutoff, 'limit', 1),
      headers := jsonb_build_object('Content-Type', 'application/json', 'x-notification-dispatcher-secret', v_secret),
...
alter function public.notification_approval_batch_submitted_dispatch_wakeup_internal() owner to postgres;
revoke all on function public.notification_approval_batch_submitted_dispatch_wakeup_internal() from public, anon, authenticated, service_role;

create trigger notification_approval_batch_submitted_dispatch_after_insert
  after insert on public.notification_events
  for each row
  when (new.event_type = 'approval_batch.submitted' and new.status = 'pending')
...
comment on function public.claim_approval_batch_submitted_events_for_dispatcher(integer, text, timestamp with time zone)
  is 'DEV claims only new approval_batch.submitted events after an exclusive activation cutoff.';
comment on function public.get_approval_batch_submitted_notification_document(uuid, text)
  is 'DEV returns the Director-scoped submitted batch snapshot for the branded email and PDF.';
comment on function public.cancel_approval_batch_submitted_event_for_dispatcher(uuid, text, text)
  is 'DEV cancels a claimed event when the batch or Director no longer matches.';
comment on function public.notification_approval_batch_submitted_dispatch_wakeup_internal()
  is 'DEV post-COMMIT wake-up for new approval_batch.submitted events only.';

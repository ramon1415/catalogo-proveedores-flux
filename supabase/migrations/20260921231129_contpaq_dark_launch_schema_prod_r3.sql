...
-- CONTPAQ dark launch PROD R3.
-- Reconstruido sobre main vigente el 2026-09-21. Aditivo y SysAdmin-only.
-- Contrato deliberado de esta primera salida:
--   * Partida -> cuenta permanece 1:1, igual que la UI actual y los 147 mapeos DEV.
--   * Tercero CONTPAQ de proveedor puede existir sin cuenta de gasto por proveedor.
--   * No incorpora todavía el grafo N:N/revisiones de DEV.
--   * No modifica RPCs de pagos, layouts, reembolsos, nómina o notificaciones.
do $pre$
begin
...
comment on table public.budget_account_mappings is
  'Dark launch R3: una sola cuenta CONTPAQ por empresa + partida. El grafo N:N de DEV no se promueve en esta fase.';
...
create table public.provider_account_mappings (
  ...
  -- La cuenta de gasto por proveedor es opcional: el tercero CONTPAQ puede
  -- existir antes del perfilamiento contable y resolver el bloque fiscal.
  contpaq_account_code text,
...
comment on column public.provider_account_mappings.contpaq_account_code is
  'Cuenta de gasto opcional por proveedor; null permite mapear primero el tercero CONTPAQ para registros fiscales.';
...
begin
  ...
  if exists (
    ...
    where table_schema='public'
      and table_name='provider_account_mappings'
      and column_name='contpaq_account_code'
      and is_nullable <> 'YES'
  ) then raise exception 'provider_account_mapping_must_allow_null_account'; end if;
  if not exists (
    select 1
    from pg_proc p join pg_namespace n on n.oid=p.pronamespace
    where n.nspname='public' and p.proname='confirm_provider_account' and not p.prosecdef
  ) then raise exception 'confirm_provider_account_must_be_security_invoker'; end if;
end
$post$;

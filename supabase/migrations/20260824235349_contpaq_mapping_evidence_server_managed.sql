-- Flux Operadora — la evidencia reproducible del seed es administrada por servidor.
-- Los usuarios pueden capturar una razón formal, pero no fabricar ni alterar
-- mapping_evidence desde el navegador.
...
BEGIN
  IF NOT v_privileged
    ...
    RAISE EXCEPTION USING
      ERRCODE = '42501',
      MESSAGE = 'contpaq_mapper_company_access_denied';
  END IF;
  ...
  IF NOT v_privileged THEN
    IF TG_OP = 'INSERT' AND NEW.mapping_evidence IS NOT NULL THEN
      RAISE EXCEPTION USING
        ERRCODE = '42501',
        MESSAGE = 'contpaq_mapping_evidence_server_managed';
    END IF;

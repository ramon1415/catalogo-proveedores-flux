...
do $precheck$
begin
  if to_regprocedure('public.approve_entire_batch(uuid)') is null
     ...
     or to_regprocedure('public.approval_batch_totals_by_currency(uuid)') is null then
    raise exception 'quick_approval_prod_prerequisites_missing';
  end if;
  if to_regclass('public.approval_batches') is null
    ...
    raise exception 'quick_approval_prod_tables_missing';
  end if;
end;
$precheck$;
...
declare
  ...
  v_worker_id text := left(coalesce(nullif(btrim(p_worker_id), ''), 'edge-approval-batch-submitted-prod'), 120);
...
begin
  ...
  if found and v_used.notification_event_id = p_notification_event_id
    ...
    return jsonb_build_object(
      ...
      'review_url', 'https://flux.quantta.mx/approval_batches.html?batch_id=' || p_batch_id::text
    );
  elsif found then
    return jsonb_build_object('state', 'invalid');
  end if;

-- Hotfix: a request confirmed as paid by a bank layout can still receive the
-- authoritative, single-page bank receipt from Comprobantes batch.
--
-- The paid exception is intentionally fail-closed. It requires one paid line
-- in one confirmed layout, one legacy/manual receipt created by that same
-- layout confirmation, a current payable snapshot, and no bank receipt link.
-- Linking preserves the existing paid request and layout timestamps/statuses.
...
do $precheck$
declare
  v_matchable_hash text;
  v_candidates_hash text;
  v_link_hash text;
begin
  select encode(digest(proc.prosrc, 'sha256'), 'hex')
    into v_matchable_hash
  from pg_proc proc
  join pg_namespace namespace on namespace.oid = proc.pronamespace
  where namespace.nspname = 'public'
    and proc.proname = 'payment_reconciliation_snapshot_is_receipt_matchable'
    and pg_get_function_identity_arguments(proc.oid) = 'p_snapshot_id uuid';

  select encode(digest(proc.prosrc, 'sha256'), 'hex')
    into v_candidates_hash
  from pg_proc proc
  join pg_namespace namespace on namespace.oid = proc.pronamespace
  where namespace.nspname = 'public'
    and proc.proname = 'find_payment_receipt_candidates'
    and pg_get_function_identity_arguments(proc.oid) = 'p_operation_id uuid, p_limit integer';
  ...
  join pg_namespace namespace on namespace.oid = proc.pronamespace
  where namespace.nspname = 'public'
    and proc.proname = 'link_payment_receipt_to_request'
    and pg_get_function_identity_arguments(proc.oid) =
      'p_operation_id uuid, p_payment_request_id uuid, p_idempotency_key text';

  if v_matchable_hash is distinct from
       '45ad9582b74cc4ae92c9a5f606bb8b872567bd96d6c5197b6a4272481557297d'
     or v_candidates_hash is distinct from
       '065dc49e727dcdb9b8a4f2c44955edde2f273269ecec2580a79c45de2c76443e'
     or v_link_hash is distinct from
       'c9f80a57b7fb7d750961f3f9912b4c1858596459eafee62471d70182c3aa70e3' then
    raise exception
      'paid_layout_receipt_match_baseline_mismatch: matchable=%, candidates=%, link=%',
      coalesce(v_matchable_hash, '<missing>'),
      coalesce(v_candidates_hash, '<missing>'),
      coalesce(v_link_hash, '<missing>');
  end if;
end
$precheck$;
...
begin
  ...
  -- Preserve the original contract for requests not yet materialized in a layout.
  ...
  if v_request.status::text = 'finance_validation' then
    ...
    -- A paid request is eligible only when payment came from exactly one
    -- confirmed layout line and has not yet received a bank evidence link.
    ...
    where legacy.payment_request_id = v_request.id
      ...
      and public.payment_amount_to_minor(legacy.amount, v_currency) = v_snapshot.amount_minor
    ...
    if v_total_legacy_receipts <> 1 or v_matching_legacy_receipts <> 1 then
      return false;
    end if;
...
begin
  ...
  if v_request.status::text not in ('approved', 'finance_validation', 'paid') then
  ...
  if v_layout_line.id is not null
     and public.payment_amount_to_minor(v_layout_line.amount, v_snapshot.currency) <>
       v_snapshot.amount_minor then
    raise exception 'layout_line_snapshot_amount_mismatch';
  end if;
  if v_legacy_receipt.id is not null
     and public.payment_amount_to_minor(v_legacy_receipt.amount, v_snapshot.currency) <>
       v_snapshot.amount_minor then
  ...
  -- Do not rewrite the original paid audit trail when Layout already confirmed it.
...
do $postcheck$
declare
  v_matchable_definition text;
  v_candidates_definition text;
  v_link_definition text;
begin
  select pg_get_functiondef(
    'public.payment_reconciliation_snapshot_is_receipt_matchable(uuid)'::regprocedure
  ) into v_matchable_definition;
  select pg_get_functiondef(
    'public.find_payment_receipt_candidates(uuid,integer)'::regprocedure
  ) into v_candidates_definition;
  select pg_get_functiondef(
    'public.link_payment_receipt_to_request(uuid,uuid,text)'::regprocedure
  ) into v_link_definition;

  if position('request.status::text in (''approved'', ''finance_validation'', ''paid'')' in
       v_candidates_definition) = 0
     or position('v_request.status::text not in (''finance_validation'', ''paid'')' in
       v_matchable_definition) = 0
     or position('if v_request.status::text <> ''paid'' then' in v_link_definition) = 0
     or position('layout.status = ''confirmed''' in v_link_definition) = 0
     or position('for update' in lower(v_link_definition)) = 0 then
    raise exception 'paid_layout_receipt_match_postcheck_failed';
  end if;

  if has_function_privilege('anon',
       'public.find_payment_receipt_candidates(uuid,integer)', 'EXECUTE')
     or not has_function_privilege('authenticated',
       'public.find_payment_receipt_candidates(uuid,integer)', 'EXECUTE')
     or not has_function_privilege('authenticated',
       'public.link_payment_receipt_to_request(uuid,uuid,text)', 'EXECUTE')
     or has_function_privilege('authenticated',
       'public.payment_reconciliation_snapshot_is_receipt_matchable(uuid)', 'EXECUTE') then
    raise exception 'paid_layout_receipt_match_acl_postcheck_failed';
  end if;
end
$postcheck$;

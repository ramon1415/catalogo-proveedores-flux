-- Strengthen future payment request decision notification dedupe.
-- Keeps one enriched notification per real recipient email, decision, and channel.
    ...
    select
      ...
      row_number() over (
        ...
        order by
          ...
          case when candidates.payload ? 'amount'
                 and candidates.payload ? 'provider'
                 and candidates.payload ? 'company'
                 and candidates.payload ? 'cost_center'
                 and candidates.payload ? 'requester'
               then 0 else 1 end,

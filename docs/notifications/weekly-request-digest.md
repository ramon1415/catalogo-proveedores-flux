...
The existing Vault dispatcher endpoint identifies the environment; its secret authenticates `weekly-request-digest`. `verify_jwt=false` is necessary for cron, and the worker validates the secret before any data access. Public RPCs are executable only by `service_role`; private tables enable RLS and revoke anonymous/authenticated access. The recipient cannot be supplied by the caller.

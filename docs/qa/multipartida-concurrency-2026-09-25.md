...
## Pendiente del paquete para main: solicitud con archivo adjunto

La conversión de intake de una sola partida y la solicitud normal con comprobante adjunto son rutas distintas. En `main`, `RequestModal` llama a `createPaymentRequestWithDocument` cuando hay archivo; esa API invoca `create_payment_request_with_document`. Las dos firmas vigentes en PROD (21 y 22 argumentos) no reciben `p_distributions`.

Antes de habilitar multi-partida en esa captura, el release debe propagar el reparto también por ese wrapper y comprobar que solicitud, distribución y enlace de archivo sean atómicos. La validación de intake no cubre esta ruta. No se modificó PROD como parte de estas pruebas.

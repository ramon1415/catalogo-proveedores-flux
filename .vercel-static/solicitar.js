...
function readyConditions() {
  const result = validation();
  return result.valid && byId("privacy-accepted").checked &&
...
function showSuccess(folio, duplicate) {
  ...
  byId("success-date").textContent = `Fecha local: ${new Intl.DateTimeFormat("es-MX", { dateStyle:"long", timeStyle:"short" }).format(new Date())}`;
  showOnly("success");

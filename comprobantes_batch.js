;(function paymentBatchUi() {
  ...
  function extractionNotice(status, issues, rejectionReason) {
    ...
    if (status === "accepted") return `<div class="notice-v2 success"><span class="notice-icon"><svg class="flux-ico" viewBox="0 0 1920 1920" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M1827.701 303.065 698.835 1431.801 92.299 825.266 0 917.564 698.835 1616.4 1919.869 395.234z"/></svg></span><span><span class="notice-title">Datos aceptados</span><span class="notice-sep">—</span><span class="notice-desc">El importe y la moneda del comprobante quedaron inmutables para el matching.</span></span></div>`
    ...
    return `<div class="notice-v2 success"><span class="notice-icon"><svg class="flux-ico" viewBox="0 0 1920 1920" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M1827.701 303.065 698.835 1431.801 92.299 825.266 0 917.564 698.835 1616.4 1919.869 395.234z"/></svg></span><span><span class="notice-title">Extracción lista para revisión</span><span class="notice-sep">—</span><span class="notice-desc">Abre el comprobante individual y valida los datos antes de aceptar.</span></span></div>`
  }
    ...
    try {
      ...
      if (!resumeExtraction) {
        ...
        if (created?.duplicate) {
          setProgress(50, "Verificando el PDF recibido anteriormente…")
          const { error: existingFinalizeError } = await client.rpc(RPC.finalizeBatch, {
            p_batch_id: batchId,
            p_page_count: parsed.page_count,
            p_idempotency_key: commandId(),
          })
          if (!existingFinalizeError) uploadFinalized = true
          else if (!isMissingPaymentBatchUpload(existingFinalizeError)) throw existingFinalizeError
        }
        if (!uploadFinalized) {
          setProgress(54, created?.duplicate ? "Reanudando carga interrumpida…" : "Subiendo al bucket privado autorizado…")
  ...
  function isMissingPaymentBatchUpload(error) {
